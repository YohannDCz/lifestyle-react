...
export const fetchArticlesBySection = async (section: string, limit: number = 4): Promise<ArticleWithContent[]> => {
  try {
...
export const fetchArticlesBySubsection = async (section: string, subsection: string, limit: number = 4): Promise<ArticleWithContent[]> => {
  try {
    const locale = getLocale()
...
export const fetchTrendingBySection = async (section: string, limit: number = 8): Promise<string[]> => {
  try {
    const locale = getLocale()
    ...
    return data?.map(article => article.article_content?.[0]?.title || 'No title') || []
